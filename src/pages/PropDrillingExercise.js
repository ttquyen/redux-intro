...
const RootComponent = (props) => {
  ...
  const handleAddProduct = (newProduct) => {
    ...
    setCart((prev) => ({
      ...
      totalPrice: prev.totalPrice + newProduct.price,
    }));
  };
  ...
  const handleRemoveProduct = (removedProduct) => {
    let canRemove = false;
    const productList = [...cart?.products].map((product) => {
      if (product.id === removedProduct.id && product.qty > 0) {
        canRemove = true;
        return {
          id: product.id,
          title: product.title,
    ...
    setCart((prev) => ({
      ...
      totalPrice: canRemove
        ? prev.totalPrice - removedProduct.price
        : prev.totalPrice,
    }));
  };
  // Step 3
        ...
        <Grid item xs={8}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <Button
              variant="error"
              sx={{ width: "5rem" }}
              onClick={() => props.handleRemoveProduct(product)}
            >
              Remove
            </Button>
          </div>
